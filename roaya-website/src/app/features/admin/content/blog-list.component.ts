...
export class BlogListComponent implements OnInit {
    ...
    this.contentService
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error('Failed to load blog posts', 'Error');
          this.loading.set(false);
        },
      });
  }
  ...
  publishItem(item: ContentItem): void {
    this.contentService.publishContent(item.id).subscribe({
      next: () => {
        this.toastService.success('Post published successfully', 'Published');
        this.loadContent();
      },
      error: () => {
        this.toastService.error('Failed to publish post', 'Error');
      },
    });
  }

  unpublishItem(item: ContentItem): void {
    this.contentService.unpublishContent(item.id).subscribe({
      next: () => {
        this.toastService.success('Post unpublished successfully', 'Unpublished');
        this.loadContent();
      },
      error: () => {
        this.toastService.error('Failed to unpublish post', 'Error');
      },
    });
  }
  ...
  deleteItem(): void {
    ...
    this.contentService.deleteContent(item.id).subscribe({
      ...
      error: () => {
        this.toastService.error('Failed to delete post', 'Error');
        this.itemToDelete.set(null);
      },
    });
  }
